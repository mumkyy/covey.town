...
/**
 * The QuantumTicTacToeArea component renders the Quantum Tic-Tac-Toe game area.
 * It renders the current state of the area, optionally allowing the player to join the game.
 *
 * It uses Chakra-UI components (does not use other GUI widgets)
 *
 * It uses the QuantumTicTacToeAreaController to get the current state of the game.
 * It listens for the 'gameUpdated' and 'gameEnd' events on the controller, and re-renders accordingly.
 * It subscribes to these events when the component mounts, and unsubscribes when the component unmounts. It also unsubscribes when the gameAreaController changes.
 *
 * It renders the following:
 * - A list of players' usernames and scores (in a list with the aria-label 'list of players in the game', one item for X and one for O)
 *    - If there is no player in the game, the username is '(No player yet!)'
 *    - List the players as (exactly) `X: ${username} (Score: ${score})` and `O: ${username} (Score: ${score})`
 ...
 * - The QuantumTicTacToeBoard component, which is passed the current gameAreaController as a prop (@see QuantumTicTacToeBoard.tsx)
 *
 ...
 *
 */
export default function QuantumTicTacToeArea({
  interactableID,
}: {
  interactableID: InteractableID;
}): JSX.Element {
  const gameAreaController =
    useInteractableAreaController<QuantumTicTacToeAreaController>(interactableID);
  ...
  const [xScore, setXScore] = useState<number>(gameAreaController.xScore);
  const [oScore, setOScore] = useState<number>(gameAreaController.oScore);
  ...
  useEffect(() => {
    const updateGameState = () => {
      ...
      setXScore(gameAreaController.xScore);
      setOScore(gameAreaController.oScore);
    };
  ...
  return (
    <Container>
      ...
      <List aria-label='list of players in the game'>
        <ListItem>
          X: {x?.userName || '(No player yet!)'} (Score: {xScore})
        </ListItem>
        <ListItem>
          O: {o?.userName || '(No player yet!)'} (Score: {oScore})
        </ListItem>
      </List>
